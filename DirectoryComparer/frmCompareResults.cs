...
using DirectoryComparer.Resources;
...
namespace DirectoryComparer
{
    public partial class frmCompareResults : Form
    {
        ...
        public frmCompareResults(string folder1path, string folder2path)
        {
            InitializeComponent();

            pathfolder1 = folder1path;
            pathfolder2 = folder2path;
        ...
        {
            PopulateDirectoryStructure loadTree = new PopulateDirectoryStructure();
            loadTree.ListDirectory(treeView1, pathfolder1);
            loadTree.ListDirectory(treeView2, pathfolder2);
        ...
        private void frmCompareResults_Load(object sender, EventArgs e)
        {
            ...
            AddItems();
            List<SyncSteps> synSteps = SetSyncSteps(Results.CoalescedResults());
            gridView.DataSource = synSteps;
            ...
            gridView.Columns[2].AutoSizeMode = DataGridViewAutoSizeColumnMode.DisplayedCells;
        ...
        private List<SyncSteps> SetSyncSteps(List<CompareResult> compareResults)
        {
            List<SyncSteps> syncSteps = new List<SyncSteps>();
            compareResults.Reverse();
            int index = 1;

            foreach(CompareResult result in compareResults)
            {
                //If the file matches, ignore it
                if (result.Match)
                {
                    continue;
                }

                //If file/folder exists only in Left Folder, Add to Right
                if(result.ExistsLeft && ! result.ExistsRight)
                {
                    SyncSteps step = new SyncSteps();
                    step.ID = index++;
                    step.Name = result.FileName;
                    step.Type = result.IsFile ? TextConstants.FILE : TextConstants.FOLDER;
                    step.Path = result.LeftFilePath;
                    step.Action = TextConstants.SYNC_ACTION_CREATE;

                    syncSteps.Add(step);
                }

                //If file/folder exists only in Righteft Folder, delete
                if (!result.ExistsLeft && result.ExistsRight)
                {
                    SyncSteps step = new SyncSteps();
                    step.ID = index++;
                    step.Name = result.FileName;
                    step.Type = result.IsFile ? TextConstants.FILE : TextConstants.FOLDER;
                    step.Path = result.RightFilePath;
                    step.Action = TextConstants.SYNC_ACTION_DELETE;
        ...
        private void InitializeList()
        {
            ...
            this.listView1.Columns.Add("Match Status").Width = 100;
            ...
            ReSizeCompareListView(listView1);
        }

        private void ReSizeCompareListView(ListView lv)
        {
